...
def save_filtered_csv(df, start_date, end_date):
    ...
    filename = f"youtube-history-{start_fmt}-to-{end_fmt}-{timestamp}.csv"
    df.to_csv(filename, index=False)
    print(f"✅ Filtered history saved to: {filename}")

def save_full_csv(df):
    timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
    filename = f"youtube-full-history-{timestamp}.csv"
    df.to_csv(filename, index=False)
    print(f"✅ Full history saved to: {filename}")
